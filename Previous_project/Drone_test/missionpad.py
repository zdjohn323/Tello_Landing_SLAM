...
def getkeyboardInput():
    ...
    az=[]
    qsp=[]
    if getKey("LEFT"): lr=-speed;y=tello.get_speed_y();print({'y dir speed',y});ay=tello.get_acceleration_y();qsp=tello.query_speed()
    elif getKey("RIGHT"): lr=speed;y=tello.get_speed_y();print({'y dir speed',y});ay=tello.get_acceleration_y();qsp=tello.query_speed()

    if getKey("UP"): fb=speed;x=tello.get_speed_x();print({'x dir speed',x});ax=tello.get_acceleration_x();qsp=tello.query_speed()
    elif getKey("DOWN"): fb=-speed;x=tello.get_speed_x();print({'x dir speed',x});ax=tello.get_acceleration_x();qsp=tello.query_speed()

    if getKey("w"): ud=speed;z=tello.get_speed_z();print({'z dir speed',z});az=tello.get_acceleration_z();qsp=tello.query_speed()
    elif getKey("s"): ud=-speed;z=tello.get_speed_z();print({'z dir speed',z});az=tello.get_acceleration_z();qsp=tello.query_speed()
    ...
    if getKey("g"):
        yaw=tello.get_yaw()    
        print(yaw)
    return [lr,fb,ud,yv,x,y,z,ax,ay,az,qsp]
...
with open("dataaz.txt","w") as f:
    f.writelines(["%s\n" % item  for item in az])
f.close
with open("dataspeed.txt","w") as f:
    f.writelines(["%s\n" % item  for item in qsp])
f.close
